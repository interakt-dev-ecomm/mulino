...
const endOfDay = async (dateObject, submitted) => {

    // // do not run if one already exists;
    // const eodPayload = {
    //     "filters": `[["date", "=", "${dateObject.processingDate}"], ["docstatus", "=", "1"]]`,
    //     "fields": `["name", "date"]`
    // }

    // const findExistingEod = await docSearch.docSearch(`End of Day`, eodPayload);

    // if (findExistingEod.length) {
    //     return;
    // }
    ...
    allDeliveryNotes.forEach(delivery => {
        deliveryTrip.push({
            ...
            "address": delivery.primary_address
        })
    })
    ...
    if (submitted === 'none') {
        ...
        const updateDoc = await docCrud.updateDoc(`End of Day`, submitted, reportPayload)
    }
